...
// - 자식 클래스에서 해당 매서드를 반드시! 구현하도록 강제하는 역할!
...
class Police extends Human { // 클래스 'Police'은(는) abstract로 선언되거나 'Human'에서 추상 메서드 'work()'을(를) 구현해야 합니다
    @Override
    void work() { // 추상 메서드 구현
...
public class A_Abstract {
    public static void main(String[] args) {
        ...
        //      >> 단, 자식만이 가진 앰버(필드 & 메서드)에 접근 불가
        Animal animalDog = new Dog();
        animalDog.makeSound();
        animalDog.eat();  // 오버라이딩된 결과가 출력
        // animalDog.bark();
    }
}
