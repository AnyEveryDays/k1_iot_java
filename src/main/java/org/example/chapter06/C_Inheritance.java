...
//      - 자식 클래스의 생성자 호출 시 '반드시' 부모 클래스의 생성자가 호출되어야한다
...
class A {
    ...
    A(int a) {
        ...
        this(); // this() 호출은  생성자 본문의 첫 번째 구문이여야 한다
        System.out.println(a);
    }
}
...
class Cat extends Mammal{
    ...
    Cat() {
        ...
        System.out.println("빈 자식 생성자 - 인스턴스 호출기");
    }
    Cat(String name){
        super(name);
...
public class C_Inheritance {
    public static void main(String[] args) {
        ...
        Cat cat2 = new Cat("나비");
        System.out.println(cat2.name);  // child가 출력이 되는데용?!?!?!?!!!???
        // 자식 이름 : Child                                // this.name = name을 안하면 child 부모값만 나오게 되는듯하다
