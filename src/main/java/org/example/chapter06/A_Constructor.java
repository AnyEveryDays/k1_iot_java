...
class Car{
    ...
    Car() {
        System.out.println("나는 빈 생성자, 사용자 정의 생성자 명시 후에는 반드시 작성해야 쓸 수 있어");
    ...
    Car(String color){
        ...
        //      >> 자신의 필드에 접근 가능
        // 2) 객체 내의 생성자 호출
        //      >> this(); - this 내부의 인지 혀앹를 컴파일러가 자동 분석하여 해당하는 생성자로 연결
        //      >> 생성자 이름으로 클래스명 대신 this를 사용

        //door = 5; // 생성을 해야 변경이 가능하다
        this(color, "auto",4);
        // cf) 사용자 정의 생성자 내에서 타 생성자 호출 시
        //      , 반드시!! 첫 줄에서만 사용 가능!!!
        // >> 호출된 생성자가 변수값을 초기화하기 때문에
        //      , 현재의 생성자가 수행하는 작업을 유지하려면 this() 생성자 뒤에 작성이 되어야한다
        //      - 무의미함 방지
...
public class A_Constructor {
    public static void main(String[] args) {
        Example1 example1 = new Example1(); // 빈 생성자 호출 - 전달할 인자가 없다.
