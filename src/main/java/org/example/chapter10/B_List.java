...
//      - 추가/제거: 공간에 대한 정리(쭝간 삽입 시 나머지 요소를 한 칸씩 이동)) >> 시간 복잡도가 발생 0(n)
//      - 꺼내기: 순차적인 수를 사용하여 꺼냄 (메모리 상의 위치로 바로 이동 가능) >> 속도 빠름

// 2) LinkedList
// : 내부에서 이중 연결 리스트를 사용한 요소 저장
// - 빠른 삽입, 삭제 성능 / 느린 읽기 성능
// - 연결 기반 데이터 관리: 비순차적 데이터 작업에 적합, 특정 요소 접근 시 순차 탑색이 필요하여 성능 저하
...
public class B_List {
    public static void main(String[] args) {
        ...
        //      : 클래스 구조는 객체 타입의 데이터만! 삽입 가능(Integer, Charactre, boolean, double 등)
