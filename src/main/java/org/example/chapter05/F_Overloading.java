...
// === (메서드) 오버로딩 === //
// : overloading - 중복 정의
// 한 클래스 내에서 동일한 이름의 '메서드'를 여러 개 정의하는 것을 의미한다
...
// 2. 오버로딩의 장점
// - 사용자가 메서드의 매개변수 순서를 외우지 않아도 사용가능
// - 작업의 본질이 동일한 메서드를 하나의 이름으로 사용(그룹화 가능)
//      >> 코드의 재사용성, 가독성 형성
// - 컴파일 시 타입 체크 강화
//      >> 올바른 매게변수를 가진 메서드를 자동 선택
...
class Calc{
    //  오버로드의 성립 조건 : 메서드명 동일, 매게변수의 타입 || 또는 반환타입은무관

    // cf) 변수는 동일한 이름의 재정의 불가
    int a = 0; // 같은 소코프 내에 동일한 이름 불가
    //long a;
    {int a;}
    int add(int a, int b){
        return a+b;
    ...
    double ad(double x, int y){return x+ y;}
